...
/**
 * Utilizes the Bidirectional Dijkstra Search Algorithm to get the search path and shortest path
 *
 ...
 */
export default function bidijkstra(grid, startSquare, endSquare) {
    /*
    ...
    */
    ...
    let k = 0;
    startFrontier.push([0, k, newGrid[startSquare[0]][startSquare[1]]]);
    k += 1;
    endFrontier.push([0, k, newGrid[endSquare[0]][endSquare[1]]]);
    ...
    while ((frontiers[0].size() > 0) && frontiers[1].size() > 0) {
        ...
        if (frontierIndex === 0) {
            if (currentSquare.rightVisited) {
                ...
                currentSquare.distance = priority;
            }
        }
        else {
        ...
        for (let [i, j] of [UP, DOWN, LEFT, RIGHT]) {
            ...
            if (frontierIndex === 0) {
                if (!(square.leftVisited || square.isWall)) {
                    k += 1;
                    frontier.push([priority + 1, k, square, currentSquare]);
                }
            }
            else {
